...
class RegisterForm(FlaskForm):
	email = StringField('Email', validators=[DataRequired(), Email()])
	password = StringField('Password', validators=[DataRequired(), Length(min=8, max=100)]) 
	password_confirm = StringField('Confirm Password', validators=[DataRequired(), Length(min=8, max=128), EqualTo('password')])
	...
	def validate_email(self, email):
		...
		if user:
			raise ValidationError('Email is already in use!')_
